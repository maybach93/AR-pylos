...
class StartedGameState: BaseGameState {
    ...
    override func movingFromPreviousState() {
        self.readyForNextStart = Observable.create({ [weak self] (observer) -> Disposable in
            ...
            self.context.gameCoordinators.keys.forEach({ self.context.gameCoordinators[$0]?.serverStateMessages.onNext(ServerMessage(type: .gameConfig, payload: GameConfigServerPayload(player: $0, map: self.context.game.map.map, stashedItems: self.context.game.stashedItems))) })
